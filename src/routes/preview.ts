...
/**
 * User-agents that get the Open Graph preview page instead of a redirect.
 *
 * These bots crawl links to generate previews when shared on social platforms.
 * A user-agent that is not listed here falls through to the redirect handler,
 * so the share card ends up showing the *destination's* metadata rather than
 * the link's.
 *
 * Every entry must be a token that ONLY a server-side fetcher sends. Bare
 * platform names are the trap: `Instagram`, `Threads`, `Snapchat` and `Tumblr`
 * all appear in those apps' in-app *browser* user-agents, so matching on the
 * name alone would serve a real person the meta-refresh interstitial instead of
 * their redirect — a worse failure than a missing preview card. They are
 * deliberately absent. Instagram and Threads previews are generated by Meta's
 * `facebookexternalhit`, which is already covered.
 *
 * `WhatsApp` below has that defect and predates this rule. Narrowing it would
 * risk breaking previews that currently work, so it stays until someone
 * captures a real WhatsApp scraper user-agent to replace it with.
 */
export const SCRAPER_PATTERNS = [
  /facebookexternalhit/i,      // Facebook, Instagram, Threads
  /Facebot/i,                  // Facebook
  /Twitterbot/i,               // X
  /LinkedInBot/i,              // LinkedIn
  /Slackbot/i,                 // Slack (also matches Slackbot-LinkExpanding)
  /Discordbot/i,               // Discord
  /TelegramBot/i,              // Telegram
  /WhatsApp/i,                 // WhatsApp — see caveat above
  /PinterestBot/i,             // Pinterest
  /SkypeUriPreview/i,          // Skype
  /Googlebot/i,                // Google (for search previews)
  /bingbot/i,                  // Bing
  /ia_archiver/i,              // Alexa
  /Applebot/i,                 // iMessage, Notes, Spotlight
  /redditbot/i,                // Reddit
  /Cardyb/i,                   // Bluesky link cards — UA masquerades as Chrome
  /Mastodon/i,                 // Fediverse
  /vkShare/i,                  // VK
  /Embedly/i,                  // Aggregator
  /Iframely/i,                 // Aggregator
  /opengraph/i,                // opengraph.xyz and similar validators
  /MetaInspector/i,            // Validator
  /PreviewBot/i,               // Generic preview fetchers
  /LinkPreview/i,              // linkpreview.net and similar
];

/**
 * Detect if the request is from a social media scraper/bot.
 */
export function isSocialScraper(userAgent: string): boolean {
